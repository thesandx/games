...
The visual and verbal rules for Playroom. They come from the Mochi design language in the `nextjs-cloudrun-template` repository, which this application was generated from. Read this **before you write or change any UI**. The tokens live in `styles/globals.css`. The primitives live in `components/ui/`. The living reference renders at `/design`.
...
Playroom uses the `playroom` theme. These decisions apply the rules above to this product. Keep them consistent.

**Primitives beyond the seven.** `components/ui/` also holds `Chip` and `RoomKeyDisplay`.

- `Chip` is a filter or a tab. A selected chip fills with `brand` and shows a check, so colour is not the only signal.
- `RoomKeyDisplay` is the room key, centred, in `text-key`. It is the only uppercase text.

**Variants added to the primitives.** Each one appears on `/design`.

- `Card flat` drops the base shadow. Use it for a card that only holds information: the rules, the player list, the round summary.
- `Avatar tone` sets the colour a player chose. The face still comes from the nickname.
- `Input` takes a `ref`, so a form can focus the field it rejected. Its error is a `role="alert"` and the field wobbles once.

**Identity tones.**

| Tone         | Means                                           |
| ------------ | ----------------------------------------------- |
| `butter`     | Bingo. A cell in a completed line. First place. |
| `soda`       | Scribble. Second place.                         |
| `grape`      | Tic-tac-toe.                                    |
| `peach`      | The number taken last. Third place.             |
| `brand-soft` | The player on turn. The winner's result card.   |

**Player colours.** The rooms API stores a player colour as `peach`, `mint`, `yellow`, `mustard` or `cream`. These names are part of the contract, so they do not change. `AVATAR_TONE` in `lib/players.ts` maps each one to a tone. `PlayerAvatar` in `components/room/` is the only place that reads it.

**The board.** A free cell you can take is the only pressable thing on the board. It has the squish. A taken cell is `bg-ink`. A free cell on another player's turn is `sunken` and dashed. The screen-reader text of each cell says its state in words.

**Moments.**

- Lobby: the mascot says who joined last.
- A rejected action or a room that cannot load: the mascot says it, with the `sad` face.
- A completed board: the "Call Bingo" card pops in once. It holds the one primary action of the round.
- Results: the winner's face peeks over the result card, with two stickers and one mascot line.
