...
- A player wins with any complete row, column, or diagonal — twelve lines. A full board is not required.
...
- With one number per turn and 25 numbers, a large room may exhaust the board before anyone completes a line. Rooms of that size should prefer fewer players or the round simply ends without a winner — no code path awards a win that is not earned.
