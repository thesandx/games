# ADR-0007: Scope the Workload Identity provider to the GitHub owner
...
This happened in `nextjs-cloudrun-template`, the template this repository comes from. Its deploy of 26 July succeeded. Its deploy of 15 September failed at the authentication step, with an identical `deploy.yml`. Playroom runs the same bootstrap script, so it has the same exposure. The fix is ported from the template (pull request #29 there).
