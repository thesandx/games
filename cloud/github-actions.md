...
3. **`attribute-condition` does not match.** It names the owner, so compare the exact owner string, including case. If it names one repository, an older bootstrap wrote it. Re-run the script. See [ADR-0007](../docs/adr/0007-scope-workload-identity-to-the-github-owner.md).
