...
describe('Avatar', () => {
  ...
  it('uses the chosen tone over the name-derived one, and keeps the face', () => {
    const { container } = render(<Avatar name="momo" tone="grape" />);
    expect(container.firstElementChild).toHaveClass('bg-grape');
    const plain = render(<Avatar name="momo" />).container;
    expect(container.querySelector('svg')?.innerHTML).toBe(plain.querySelector('svg')?.innerHTML);
  });
});
