...
describe('Input', () => {
  ...
  it('announces the error the moment it appears', () => {
    render(<Input label="Room key" error="Keys are 6 letters or numbers." />);
    expect(screen.getByRole('alert')).toHaveTextContent('Keys are 6 letters or numbers.');
  });

  it('hands its ref to the field, so a form can focus what it rejected', () => {
    const ref = createRef<HTMLInputElement>();
    render(<Input label="Nickname" ref={ref} />);
    expect(ref.current).toBe(screen.getByLabelText('Nickname'));
  });
});
