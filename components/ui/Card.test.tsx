...
describe('Card', () => {
  ...
  it('sits on a base by default, and flat when it only holds information', () => {
    render(
      <>
        <Card>Pressable</Card>
        <Card flat>Rules</Card>
      </>,
    );
    expect(screen.getByText('Pressable')).toHaveClass('shadow-mochi');
    expect(screen.getByText('Rules')).not.toHaveClass('shadow-mochi');
  });
});
