#!/usr/bin/env bash
#
# Deploy Firestore security rules, composite indexes, field exemptions and TTL
# policies from the repository.
#
...
deploy_indexes() {
  ...
  while IFS= read -r override; do
    ...
    if [[ "$(jq -r '.ttl // false' <<<"$override")" == "true" ]]; then
      deploy_ttl "$collection_group" "$field_path"
    fi
...
# TTL policies
#
# `"ttl": true` on a field override means "delete a document once the
# Timestamp in this field has passed". Firestore does the deleting, usually
# within a day of expiry, and the deletes are not billed as writes. The app
# never relies on the timing: it treats an expired document as gone on read.
#
# Enabling a policy is a long-running operation, so this starts it and moves
# on (--async). A policy that is already there is left alone.
# ---------------------------------------------------------------------------
deploy_ttl() {
  local collection_group="$1" field_path="$2" state output
  state="$(gcloud firestore fields ttls list \
      --project="$PROJECT_ID" \
      --database="$DATABASE_ID" \
      --collection-group="$collection_group" \
      --format=json 2>/dev/null \
    | jq -r --arg field "$field_path" \
      '[.[] | select(.name | endswith("/fields/" + $field)) | .ttlConfig.state] | first // empty')"

  if [[ "$state" == "ACTIVE" || "$state" == "CREATING" ]]; then
    skip "TTL on ${collection_group}.${field_path} is already ${state}"
    return 0
  fi
  ...
  if output="$(gcloud firestore fields ttls update "$field_path" \
        ...
        --async \
        --quiet 2>&1)"; then
    ok "Enabling TTL on ${collection_group}.${field_path} (runs in the background)"
  else
    printf '%s\n' "$output" >&2
    die "Failed to enable TTL on ${collection_group}.${field_path}"
  fi
}
